...
// نام سوره را به قرمز/آبی نفتی دوقرنگ می‌کنیم
...
// نام لاتین سوره‌ها برای تیتر اصلی به سبک QUraan FM ("SURAH YASEEN")
const SURAH_EN: Record<string, string> = {
  'الرعد': 'AR-RAD', 'بقره': 'AL-BAQARAH', 'طلاق': 'AT-TALAQ',
  'انشراح': 'ASH-SHARH', 'زمر': 'AZ-ZUMAR', 'فجر': 'AL-FAJR',
  'طه': 'TA-HA', 'انفال': 'AL-ANFAL', 'فلق': 'AL-FALAQ',
  'اخلاص': 'AL-IKHLAS', 'عصر': 'AL-ASR', 'فتح': 'AL-FATH',
  'ضحی': 'AD-DUHA', 'هود': 'HUD', 'آلعمران': 'AL-IMRAN',
  'اسراء': 'AL-ISRA', 'غافر': 'GHAFIR', 'جمعه': 'AL-JUMUAH',
  'نساء': 'AN-NISA', 'عنکبوت': 'AL-ANKABUT', 'نجم': 'AN-NAJM',
  'حدید': 'AL-HADID', 'ملک': 'AL-MULK', 'توبه': 'AT-TAWBAH',
  'انبیاء': 'AL-ANBIYA', 'الرحمن': 'AR-RAHMAN', 'انعام': 'AL-ANAM',
  'نمل': 'AN-NAML', 'کهف': 'AL-KAHF', 'سجده': 'AS-SAJDAH',
  'سبأ': 'SABA',
};
export const surahEn = (name: string): string => SURAH_EN[name] || name.toUpperCase();

const RECITER_EN = 'YASSER ALDOSARI';
...
// آیکون‌های تماس (واتس‌اپ / اینستاگرام / فیسبوک) به‌صورت SVG ساده
...
const ContactBanner: React.FC = () => {
  ...
  return (
    ...
    >
      {/* نشان سفید با نماد هندسی آبی/طلایی */}
      <div
        style={{
          width: 96,
          height: 96,
...
export const Thumbnail: React.FC<{index: number}> = ({index}) => {
  const frame = useCurrentFrame();
  const src = staticFile('/backgrounds/bg_mountains.mp4');
  const drift = interpolate(frame, [0, 60], [1, 1.07], {extrapolateRight: 'clamp'});
  const item = ayahs[index];
  const surah = surahEn(item.surah_name || '');
  const surahParts = splitSurah(surah);
  const ayahNo = String(item.ayah || '').split('-')[0];
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#0d0a05'}}>
      ...
      />
      {/* پنل روشن ملایم پشت تیتر برای وضوح بهتر */}
      <AbsoluteFill
        style={{
      ...
      {/* تیتر اصلی مرکز-چپ */}
      <div style={{position: 'absolute', left: 64, top: 210, display: 'flex', flexDirection: 'column'}}>
        <div style={{fontFamily: Fonts.sans, fontWeight: 500, fontSize: 40, color: '#1A1207', letterSpacing: 14, textShadow: '0 2px 4px rgba(255,255,255,0.7)'}}>
          SURAH
        </div>
        <div style={{display: 'flex', alignItems: 'flex-end', gap: 20, marginTop: 2}}>
          <div style={{display: 'flex', fontFamily: Fonts.sans, fontWeight: 900, fontSize: 150, lineHeight: 1, letterSpacing: 1, textShadow: '0 3px 0 rgba(255,255,255,0.5), 4px 6px 0 rgba(255,255,255,0.3), 0 14px 34px rgba(0,0,0,0.4)'}}>
        ...
        <div style={{fontFamily: Fonts.sans, fontWeight: 700, fontSize: 34, color: '#241805', letterSpacing: 6, marginTop: 6, textShadow: '0 2px 4px rgba(255,255,255,0.65)'}}>
          {RECITER_EN}
        </div>
